---
namespace: personal
type: [insight, observation, task]
date-created: 2026-04-19
source-file: raw/Thoughts.md, raw/Thoughts 2.md
---

# Learning and Work Habits

Patterns in how I learn and work — what's helping, what needs fixing.
Source: raw/Thoughts 2.md (pts 1, 2, 8, 9, 18, 25, 28, 29, 32) · raw/Thoughts.md (pts 3, 4, 9, 11, 15, 19, 36, 37, 41, 58, 62, 63, 70)

---

## AI Dependency in Coding (point 1)
I don't struggle with code before going to AI — I spot a doubt and go straight to AI. This is efficient but may be preventing me from building independent problem-solving muscle. See [[identity-and-impulse-control]] for the office-AI strategy angle.

## Completion Over Understanding (point 2)
I focus on finishing each module as a goal rather than genuinely understanding the topic. Speed feels productive but risks shallow knowledge. Connects to [[work-and-career]] — upskilling requires depth, not just throughput.

## Physical Health at Desk (point 8)
Current risks: neck pain, knee damage from folding legs while seated, back problems from long hours.
Action: Extend the 20:20:20 eye rule into a full body rule — regular posture breaks, leg uncrossing, back checks. This needs a system, not willpower.

## Saying Yes on Impulse — Social Commitments (point 9)
Said yes to a friend outing on impulse. Couldn't say no to boss for extra work. Need to balance friendship, networking, and work without defaulting to yes reflexively. See [[identity-and-impulse-control]] for the broader impulse control theme.

## Morning Lust and Net Surfing (point 18)
Triggered by solitude — brain reads "alone = no risk of getting caught." Need a pre-emptive system for mornings, not a reactive willpower fight. See [[identity-and-impulse-control]].

## Data Science Learning Pace (point 25)
One week in — feeling like I'm copying the instructor rather than creating. Pace feels slow but I'm prioritising understanding over speed. This is actually correct early-stage behaviour; the creative ownership will come. Watch this over the next few weeks.

---
*Added from raw/Thoughts.md and raw/Thoughts 2.md — 2026-04-19*

## Passive Reading in the CAM Project (Thoughts 2 pt 28)
After each answer Claude gave, I wasn't struggling with it — just passively reading. That's why I couldn't explain to MD. Active reading means: stop after each point, ask yourself if you could explain it, and if not, re-engage until you can. Passive consumption feels like learning but doesn't stick.

## Eating and Medicine Habit (Thoughts pt 3)
Need to control eating and then taking medicine to make up for it. The medicine is treating the symptom, not the cause. Stop the eating that requires the medicine. See [[lust-control-system]] — the same impulse control framework applies here.

## Internet Addiction (Thoughts pt 4)
Need to control internet addiction as a core habit. See the negotiation strategy in [[identity-and-impulse-control]] (pt 17) and the no-net-before-sleep rule (pt 18).

## Dwarka Trip Proof (Thoughts pt 9)
I controlled eating on the Dwarka trip for an extended period. This is evidence that I can do it — not just for a day, but sustained. Reference this when the inner critic says "I can't control this." I did it before. I can do it again.

## Daily Diary Not Started (Thoughts pt 11)
Haven't started the daily diary yet — keep leaving it out of laziness. This is a foundational habit that feeds everything else. The barrier is low; the resistance is avoidance. Start tonight, not when the "right moment" comes.

## Losing and Forgetting Things (Thoughts pts 15, 58)
Pattern: lose things when tired or when mind is very occupied (e.g. during a test). Solution: in those states specifically, keep everything together in or near the bag immediately — don't rely on memory to retrieve it later. Also: once a month, do a deep clean of shelves and spaces. You'll find things you forgot you owned (Thoughts pt 70).

## Cover Mouth When Eating While Talking (Thoughts pt 36)
Basic etiquette that needs to become automatic.

## Do Every Task Then and There (Thoughts pt 37)
However small the task — do it now. Postponement becomes infinite. Small deferred tasks accumulate into a background anxiety that's heavier than the tasks themselves.

## Current Progress Snapshot (Thoughts pt 41)
Doing well on: 6am wakeup, net control, balancing CS study with MBA. These are the current wins. Build on them. Next to tackle: bad eating habits, calling relatives (especially Dadu), and the full physical health routine (20:20:20, stand every 1 hour, etc.).

## Sleep Routine (Thoughts pt 62)
Prioritise sleep over late-night work. Target: decompress by 10pm (anime counts), asleep by 11pm strict. The sleep routine includes: clean and prepare the bed beforehand — otherwise it itches at night. A clean bed is not optional.

## Chewing Properly (Thoughts pt 63)
Practise proper chewing from now. Not doing so in your 20s leads to digestive problems in old age. Simple, free, and easy to forget.

---
*Added from raw/Thoughts 2.md — 2026-04-25*

## Stop Overthinking — Simplicity First (Thoughts 2 pt 29)
Piyush is right: I overthink and overcomplicate. While I was worrying about document variations and edge cases, he simply said — brute force it with Claude and understand as you go. This is a pattern: I reach for the elegant solution before I understand the basic one. The fix is embarrassingly simple: start with the dumbest working approach, understand it fully, then refine. Complexity is a reward for understanding, not a starting point.

## Energy Drain from Parallel Worrying (Thoughts 2 pt 32)
I become especially tired because I focus on work AND simultaneously worry about self-improvement, habits, goals, and everything else. The Silent Monk video frames this clearly: this internal noise quietly drains energy throughout the day. The fix is not to stop caring about self-improvement — it's to give it its designated time (journalling, Sunday review per [[master-regulators]]) and then genuinely let it go during work hours. Quietly bring the brain back to the task at hand. This also helps with anxiety and tension.

---
*Connected pages: [[identity-and-impulse-control]] · [[master-regulators]] · [[lust-control-system]] · [[work-and-career]] · [[sequencing-and-systems]] · [[physical-health-habits]]*
